...
*/
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, Hal::I2c *twi,
...
void Adafruit_SSD1306::ssd1306_commandList(const uint8_t *c, uint16_t n, uint8_t emptyByte)
{
  //printf("ssd1306_commandList Len: %d\n", n);
  if (wire)
  { // I2C
    ...
    for (uint8_t i = 0; i * WIRE_MAX < n; i++)
    {
      uint8_t data[WIRE_MAX + 1] = {};
      data[0] = emptyByte;
      int min = std::min(n - WIRE_MAX * i, WIRE_MAX);
...
void Adafruit_SSD1306::display(void)
{
  ...
  { // I2C
    ...
    // }
    // wire->endTransmission();
    ssd1306_commandList(ptr, count, 0x40);
  }
  else
  { // SPI
    SSD1306_MODE_DATA
